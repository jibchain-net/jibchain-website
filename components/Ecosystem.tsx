              ...
              <JibCard key={tier.name}>
                <div className="grid grid-cols-1 justify-items-start">
                  ...
                  <div className='text-lg font-semi-bold p-3'>
                    <span className='text-white text-2xl'>{tier.name}</span>
                    <div>
                      {tier.tag.map((tag) => (
                        <span className="inline-flex mr-3 my-3 bg-secondary-200 items-center gap-x-1.5 rounded-md px-2 py-1 text-xs font-medium text-secondary-100 ">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <p className='text-secondary-100'>{tier.description}</p>
                  </div>
                </div>
              </JibCard>
            ))}
